...
change_in_specialties %>% 
  ...
  theme_classic() +
  scale_y_continuous(labels = percent_format())+
  scale_fill_manual(values = c("FALSE" = "seagreen", "TRUE"= "tomato"))+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 8,angle = 45, hjust = 1),
        legend.position = "none") +
  labs(y = "Percentage Increase (%)",
       title = "Top 5 changes in hospital admissions (by specialty) - pre-Covid vs Covid",
       subtitle = "increase and decrease")
...
geom_col(aes(x = reorder(specialty_name, value, decreasing = TRUE),
             y=value, color=variable, fill = variable), position = "dodge")+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size = 8,angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("pre_covid_year_prop" = "grey", "covid_year_prop"= "tomato"))+
  scale_colour_manual(values = c("pre_covid_year_prop" = "grey", "covid_year_prop"= "tomato"))+
  scale_y_continuous(labels = percent_format())+
  labs(y = "Percentage (%)",
       title = "Top 5 specialty in hospital admissions - pre-Covid vs Covid")
